...
const getEnvVar = (key: string, defaultValue?: string): string => {
  ...
  if (!value) {
    throw new InternalServerError(`Environment variable ${key} is required but not defined`);
  }
  return value;
};
...
const getEnvVarAsNumber = (key: string, defaultValue?: number): number => {
  ...
  if (isNaN(parsed)) {
    throw new InternalServerError(`Environment variable ${key} must be a valid number`);
  }
