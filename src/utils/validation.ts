  ...
  for (const rule of rules) {
    ...
    if (typeof value === 'string') {
      ...
      if (maxLength && value.length > maxLength) {
        if (!errors[field]) errors[field] = [];
        errors[field].push(`${field} must be no more than ${maxLength} characters long`);
      }
    }
    ...
    if (custom && value !== undefined && value !== null) {
      ...
      if (customResult !== true) {
        if (!errors[field]) errors[field] = [];
        errors[field].push(typeof customResult === 'string' ? customResult : `${field} is invalid`);
      }
    }
  }
...
export const validateIds = (ids: any): number[] => {
  ...
  const numericIds = ids.map((id) => {
    ...
    if (isNaN(numId) || numId <= 0) {
      throw new ValidationError(`Invalid ID: ${id}. IDs must be positive numbers`);
    }
    return numId;
  });
...
export const validateId = (id: any): number => {
  const numId = Number(id);
  if (isNaN(numId) || numId <= 0) {
    throw new ValidationError(`Invalid ID: ${id}. ID must be a positive number`);
  }
  return numId;
};
